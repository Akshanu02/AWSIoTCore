...
parser.add_argument('--certificates-path', dest='certificates_path', default='/home/ec2-user/environment/AWSIoTCore', 
                    type=str, required=False, help='Modify the cert folder path if your certificates are in another folder')

args = parser.parse_args()

certs = {
    "cafile": args.certificates_path+"/rootCA.pem",
    "certfile": args.certificates_path+"/certificate1.pem",
    "keyfile": args.certificates_path+"/privatekey1.key",
}

def on_subscribe(mqttc, userdata, mid, granted_qos, properties=None):
    # Publish a plain text payload to topic "car01/main_door/lock"
    properties = Properties(PacketTypes.PUBLISH)
    pub_topic = "car01/main_door/lock"
    ...
    properties.ResponseTopic = "car01/main_door/status"
    payload = "LOCK"
...
def on_connect(mqttc, userdata, flags, reasonCode, properties=None):
    mqttc.subscribe('car01/main_door/#', qos=0)
...
def on_message(mqttc, userdata, message):
    ...
    if message.topic == "car01/main_door/lock":
    ...
    elif message.topic == "car01/main_door/status":
